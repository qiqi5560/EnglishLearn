...
| LLM 大模型对话 | AI 角色扮演、场景对话陪练，并给出语法/用词改进建议 | **Ollama + qwen2.5:7b-instruct**（本地运行） |
...
## 5. 局域网共享大模型（组员连组长电脑，无需各自下载 7B 模型）
...
> 组员**不需要**下载 `qwen2.5:7b-instruct`（省 4.6GB），也**不需要**本机装 Ollama 模型。
...
> 注意：**同一时间只能组长一台机器跑模型**，多人并发提问会排队变慢（7B 模型 CPU 推理约数秒一条）。
> 演示时建议一个一个来，或提前打好招呼。
...
| 大模型回复慢 | 本机无独显，7B 模型 CPU 推理约数秒一条属正常；演示够用 |
